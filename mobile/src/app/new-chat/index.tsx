...
const NewChatScreen = () => {
  ...
  const handleUserSelect = (user: User) => {
    console.log("Clicked user:", user.name); // 1. Verify click works

    getOrCreateChat(user._id, {
      onSuccess: (chat) => {
        console.log("Chat created successfully:", chat._id); // 2. Verify success
      ...
      onError: (error) => {
        // 3. Catch and log any API/Network errors
